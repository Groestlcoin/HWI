...
def process_commands(cli_args):
    ...
    getmasterxpub_parser = subparsers.add_parser('getmasterxpub', help='Get the extended public key at m/44\'/17\'/0\'')
    ...
    getkeypool_parser = subparsers.add_parser('getkeypool', help='Get JSON array of keys that can be imported to groestlcoin Core with importmulti')
    ...
    kp_type_group.add_argument('--sh_wpkh', action='store_true', help='Generate p2sh-nested segwit addresses (default path: m/49h/17h/0h/[0,1]/*)')
    kp_type_group.add_argument('--wpkh', action='store_true', help='Generate bech32 addresses (default path: m/84h/17h/0h/[0,1]/*)')
    ...
    getkeypool_parser.add_argument('--path', help='Derivation path, default follows BIP43 convention, e.g. m/84h/17h/0h/1/* with --wpkh --internal. If this argument and --internal is not given, both internal and external keypools will be returned.')
