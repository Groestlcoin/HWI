...
class LedgerClient(HardwareWalletClient):
    ...
    def sign_tx(self, tx: PSBT) -> PSBT:
        ...
        The scripts supported depend on the version of the Groestlcoin Application installed on the Ledger.
    ...
    def display_singlesig_address(
        ...
        if isinstance(self.client, LegacyClient):
            if addr_type == AddressType.LEGACY:
                ...
                BadArgumentError("Taproot is not supported by this version of the Groestlcoin App")
            else:
                BadArgumentError("Unknown address type")
    ...
    def display_multisig_address(
        ...
        if isinstance(self.client, LegacyClient):
            raise BadArgumentError("Displaying multisignature addresses is not supported by this version of the Groestlcoin App")
        ...
        if any(not is_valid_der_path(pk.deriv_path) for pk in multisig.pubkeys):
            raise BadArgumentError("Ledger Groestlcoin app requires derivation paths ending with /0/* or /1/* for multisig")

        if not (all(pk.deriv_path == multisig.pubkeys[0].deriv_path for pk in multisig.pubkeys)):
            raise BadArgumentError("Ledger Groestlcoin app requires all derivation paths to end with /0/*, or all with /1/* for multisig")

        if any(pk.origin is not None and len(pk.origin.path) > 4 for pk in multisig.pubkeys):
            raise BadArgumentError("Ledger Groestlcoin app requires extended keys with derivation length at most 4")
    ...
    def can_sign_taproot(self) -> bool:
        """
        Ledgers support Taproot if the Groestlcoin App version greater than 2.0.0; support here is for versions 2.1.0 and above.

        :returns: True if Groestlcoin App version is greater than or equal to 2.1.0, and not the "Legacy" release. False otherwise.
        """
        ...
        if ('interface_number' in d and d['interface_number'] == 0
            ...
            with handle_errors(common_err_msgs["enumerate"], d_data):
                try:
                    ...
                    # This only happens if the ledger is not in the Groestlcoin app, so skip it
                    continue
