# Using Groestlcoin Core with Hardware Wallets

This approach is fairly manual, requires the command line, and Groestlcoin Core >=2.18.2.
...
This method of using hardware wallets uses Groestlcoin Core as the wallet for monitoring the blockchain. It allows a user to use their own full node instead of relying on an SPV wallet or vendor provided software.

HWI works with Groestlcoin Core >=2.18.2.

## Setup

Clone Groestlcoin Core and build it. Clone HWI.
...
```
$ ./hwi.py enumerate
[{"fingerprint": "8038ecd9", "serial_number": "205A32753042", "type": "trezor", "path": "0001:0005:00"}]
```

For this example, we will use the Trezor. As we can see, the device path is `0001:0005:00`. The fingerprint of the master key is `8038ecd9`. Now that we have the device, we can issue commands to it. So now we want to get some keys and import them into Core.
...
[{"desc": "wpkh([8038ecd9/84h/17h/0h]xpub6DR4rqx16YnCcfwFqgwvJdKiWrjDRzqxYTY44aoyHwZDSeSB5n2tqt42aYr9qPKhSKUdftPdTjhHrKKD6WGKVbuyhMvGH76VyKKZubg8o4P/0/*)#36sal9a4", "internal": false, "range": [0, 1000], "timestamp": "now", "keypool": true, "watchonly": true}, {"desc": "wpkh([8038ecd9/84h/17h/0h]xpub6DR4rqx16YnCcfwFqgwvJdKiWrjDRzqxYTY44aoyHwZDSeSB5n2tqt42aYr9qPKhSKUdftPdTjhHrKKD6WGKVbuyhMvGH76VyKKZubg8o4P/1/*)#nl2rc26w", "internal": true, "range": [0, 1000], "timestamp": "now", "keypool": true, "watchonly": true}]
```

We now create a new Groestlcoin Core wallet and import the keys into Groestlcoin Core. The output is formatted properly for Groestlcoin Core so it can be copy and pasted.

```
$ ../groestlcoin/src/groestlcoin-cli createwallet "trezor" true
{
  "name": "trezor",
  "warning": ""
}
$ ../groestlcoin/src/groestlcoin-cli -rpcwallet=trezor importmulti '[{"desc": "wpkh([8038ecd9/84h/17h/0h]xpub6DR4rqx16YnCcfwFqgwvJdKiWrjDRzqxYTY44aoyHwZDSeSB5n2tqt42aYr9qPKhSKUdftPdTjhHrKKD6WGKVbuyhMvGH76VyKKZubg8o4P/0/*)#36sal9a4", "internal": false, "range": [0, 1000], "timestamp": "now", "keypool": true, "watchonly": true}, {"desc": "wpkh([8038ecd9/84h/17h/0h]xpub6DR4rqx16YnCcfwFqgwvJdKiWrjDRzqxYTY44aoyHwZDSeSB5n2tqt42aYr9qPKhSKUdftPdTjhHrKKD6WGKVbuyhMvGH76VyKKZubg8o4P/1/*)#nl2rc26w", "internal": true, "range": [0, 1000], "timestamp": "now", "keypool": true, "watchonly": true}]'
...
The Groestlcoin Core wallet is now setup to watch two thousand keys (1000 normal, 1000 change) from your hardware wallet and you can use it to track your balances and create transactions. The transactions will need to be signed through HWI.
...
```
$ ../groestlcoin/src/groestlcoin-cli rescanblockchain <blockheight>
$ ../groestlcoin/src/groestlcoin-cli rescanblockchain 500000 # Rescan from block 500000

$ ../groestlcoin/src/groestlcoin-cli -rpcwallet=trezor importmulti '[{"internal": true, "timestamp": <blockheight>, "desc": "wpkh([8038ecd9/84h/17h/0h]xpub6DR4rqx16YnCcfwFqgwvJdKiWrjDRzqxYTY44aoyHwZDSeSB5n2tqt42aYr9qPKhSKUdftPdTjhHrKKD6WGKVbuyhMvGH76VyKKZubg8o4P/1/*)#qw4uzsdd", "keypool": true, "range": [0, 1000], "watchonly": true}]'
$ ../groestlcoin/src/groestlcoin-cli -rpcwallet=trezor importmulti '[{"internal": true, "timestamp": 500000, "desc": "wpkh([8038ecd9/84h/17h/0h]xpub6DR4rqx16YnCcfwFqgwvJdKiWrjDRzqxYTY44aoyHwZDSeSB5n2tqt42aYr9qPKhSKUdftPdTjhHrKKD6WGKVbuyhMvGH76VyKKZubg8o4P/1/*)#qw4uzsdd", "keypool": true, "range": [0, 1000], "watchonly": true}]' # Imports and rescans from block 500000
```

## Usage

Usage of this primarily involves Groestlcoin Core. Currently the GUI only supports generating new receive addresses (once all of the keys are imported) so this guide will only cover the command line.

### Receiving

From the folder containing `groestlcoin` and `HWI`, go into `groestlcoin`. We will be doing most of the commands here.
...
```
$ src/groestlcoin-cli -rpcwallet=trezor getnewaddress
grsrt1qu8qe24zq5e2ahh4nkl6g5ysxlpn3nyf0xt026s
```

This address belongs to your hardware wallet. You can check this by doing `getaddressinfo`:

```
$ src/groestlcoin-cli -rpcwallet=trezor getaddressinfo grsrt1qu8qe24zq5e2ahh4nkl6g5ysxlpn3nyf0xt026s
{
  "address": "grsrt1qu8qe24zq5e2ahh4nkl6g5ysxlpn3nyf0xt026s",
...
You can give this out to people as you normally would. When coins are sent to it, you will see them in your Groestlcoin Core wallet as watch-only.
...
```
$ src/groestlcoin-cli decodepsbt 
{
  "tx": {
    ...
    "vout": [
      {
        ...
        "scriptPubKey": {
          ...
          "addresses": [
            "grs1q257z5t76hedc36wmmzva05890ny3kxd7xfwrgy"
          ]
        }
      },
      {
        ...
        "scriptPubKey": {
          ...
          "type": "witness_v0_keyhash",
          "addresses": [
            "grs1qk8h97av3hran0j5hjqan3rwrn2zegy0unusy49"
          ]
        }
      }
    ]
  ...
  "inputs": [
    {
      "witness_utxo": {
        ...
        "scriptPubKey": {
          ...
          "type": "witness_v0_keyhash",
          "address": "grs1qu8qe24zq5e2ahh4nkl6g5ysxlpn3nyf0wyd5k2"
        }
      },
      "bip32_derivs": [
        {
...
We can now take the PSBT, finalize it, and broadcast it with Groestlcoin Core

```
$ cd ../groestlcoin
$ src/groestlcoin-cli finalizepsbt 
{
...
$ src/groestlcoin-cli sendrawtransaction 
...
HWI follows BIPs 44, 84, and 49. By default, descriptors will be for P2PKH addresses with keys derived at `m/44h/17h/0h/0` for normal receiving keys and `m/44h/17h/0h/1` for change keys.
Using the `--wpkh` option will result in P2WPKH addresses with keys derived at `m/84h/17h/0h/0` for normal receiving keys and `m/84h/17h/0h/1` for change keys.
...
To actually get the correct address type when using `getnewaddress` from Groestlcoin Core, you will need to additionally set `-addresstype=p2sh-segwit` and `-changetype=p2sh-segwit`.
This can be set in the command line (as shown in the example) or in your groestlcoin.conf file.
