...
```
$ ./hwi.py -f 8038ecd9 getkeypool 0 1000
...
HWI follows BIPs 44, 84, and 49. By default, descriptors will be for P2WPKH addresses with keys derived at `m/84h/17h/0h/0` for normal receiving keys and `m/84h/17h/0h/1` for change keys.
Using the `--addr-type legacy` option will result in P2PKH addresses with keys derived at `m/44h/17h/0h/0` for normal receiving keys and `m/44h/17h/0h/1` for change keys.
